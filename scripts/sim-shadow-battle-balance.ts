...
const CASES: Array<{ id: string; shadowIds: string[] }> = [
...
console.log('# 12-11 Shadow Battle Balance')
...
for (const build of BUILDS) {
  ...
  for (const testCase of CASES) {
    const shadows = makeShadows(testCase.shadowIds)
    ...
    for (const gateId of GATE_IDS) {
      ...
      const summary = summarizeGateWaveBattleSimulations({
        ...
        seedBase: SEED_BASE + build.id.charCodeAt(0) * 10_000 + testCase.id.length * 100 + gate.id.length,
      })
      const notes: string[] = []
